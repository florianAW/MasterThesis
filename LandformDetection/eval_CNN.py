...
df_data = load_image_data(resize=True, resize_dim=(128,128), normalise=True, multiclass=True)
...
if not MULTICLASS:
    ...
    ### load u-net model
    unet_model = keras.models.load_model('best_model_binary.h5')
    ...
    ### load u-net model
    unet_model = keras.models.load_model('best_model_multiclass.h5')
            ...
            for x in range(len(predicted_mask[0][0])):
                for key, value in colors.items():
                    if predicted_mask[z][y][x][0] == key:
                        predicted_mask[z][y][x] = value # Fill vector with respected color (e.g. Crater (5) = (0,0,255))
